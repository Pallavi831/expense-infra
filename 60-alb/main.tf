...
module "alb" {
  source  = "terraform-aws-modules/alb/aws"
  version = "~> 8.0" # specify version for stability
  ...
  # Let the module create the security group
...
resource "aws_lb_listener" "https" {
  ...
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-2016-08"
  certificate_arn   = local.ingress_alb_certificate_arn # <-- must be valid in ALB's region (e.g. us-east-1)
